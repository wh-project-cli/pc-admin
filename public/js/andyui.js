...
(function (window) {
	...
	module.miniDialog = function (options) {
		...
		var body = $('body');
		body.find('.m-alertbox').length > 0
			? null
			: body.append("<div class = 'm-alertbox t-m'></div>");
	...
	module.startCheck = function (str) {
		...
		debug == 'open'
			? (module.checkHanld = setInterval(function () {
					module.costTime += 1;
			  }, 1))
			: null;
	};
...
(function (window) {
	...
	module.floatbox = function (a) {
		if (a.is('.top')) {
			if (a.is(':hidden')) {
				...
				var floath = a.outerHeight();
				a.animate({ top: 0 - floath - 5 + 'px' }, 300, function () {
					a.hide();
				});
			}
		}
		...
		if (a.is('.bottom')) {
			if (a.is(':hidden')) {
				...
				var floath = a.outerHeight();
				a.animate({ bottom: 0 - floath - 5 + 'px' }, 300, function () {
					a.hide();
				});
			}
		}
		if (a.is('.left')) {
			if (a.is(':hidden')) {
				...
				a.animate({ left: 0 - floath - 5 + 'px' }, 300, function () {
					a.hide();
				});
			}
		}
		...
		if (a.is('.right')) {
			if (a.is(':hidden')) {
				...
				var floath = a.outerWidth();
				a.animate({ right: 0 - floath - 5 + 'px' }, 300, function () {
					a.hide();
				});
			}
		}
	...
	module.perform = function (e) {
		...
		if (body.children('div,input,form,table,thead,tbody,th,tr,td,img,ul,li,a').length > 0) {
			body.children('div,input,form,table,tbody,th,tr,td,img,ul,li,a').each(function (index, cell) {
				...
				// 屏蔽隔断判断
				if (
					!cell.hasClass(andy.UI_SHIELDING) &&
					!cell.hasClass(andy.UI_SHIELDING_PERFORM) &&
					!cell.is('[autoxmpcode=true]')
				) {
					andy.perform(cell);
				}
			});
	...
	module.layout = function (e) {
		...
		body.each(function (index, element) {
			...
			if ($element.parent().is('body')) {
				//新增在超限屏幕下,其父级高宽等于CSS媒体查询限制高宽
				if ($(window).width() < 1366) {
					parentWidth = $('body').width();
					parentHeight = $('body').height();
				} else {
			...
			if ($element.children('div').length > 0) {
				...
				$element.children('div').each(function (i, el) {
					var div = $(el);
					if (
						!div.hasClass('f-hidden') &&
						!div.hasClass('item') &&
						!div.hasClass(andy.UI_SHIELDING) &&
						!div.hasClass(andy.UI_SHIELDING_LAYOUT) &&
						!div.is('[autoxmpcode=true]')
					) {
						andy.layout(div);
					}
				});
				...
				$element.children('table,form').each(function (i, el) {
					...
					if (
						!cell.hasClass(andy.UI_SHIELDING) &&
						!cell.hasClass(andy.UI_SHIELDING_LAYOUT) &&
						!cell.is('[autoxmpcode=true]')
					) {
						andy.layout(cell);
						// 渲染表单问题 待处理
	...
	module.tabsLayout = function (tabs) {
		...
		var headWidth = 0;
		if (
			!tabs.hasClass(_stringclass.headTop) &&
			!tabs.hasClass(_stringclass.headLeft) &&
			!tabs.hasClass(_stringclass.headRight) &&
			!tabs.hasClass(_stringclass.headBottom)
		) {
	...
	module.panelauto = function () {
		$('.u-panel.u-datagrid').each(function (index, element) {
			var upanel = $(element);
			var Pw = upanel.parent().width(),
				Ph = upanel.parent().height();
			upanel.height(Ph);
		...
		$('.u-panel.window').each(function (index, element) {
			...
			var Pw = upanel.parent().width(),
				Ph = upanel.parent().height() - padding_top - padding_bottom;
		...
		$('.m-table-form.inline')
			...
			.each(function (index, element) {
				...
				$element
					.parent()
					.parent()
					.find('.u-input-span, .u-input, textarea, .g-combo')
					.each(function (i, e) {
						// 获取到所有TD内容的 最大高度
						var cell = $(e);
						if (maxHeight == 0) {
	...
	module.inputLayout = function (input) {
		...
		input.siblings().each(function (j, element) {
			...
			var other = $(element);
			if (
				other.hasClass('infobox') == false &&
				other.hasClass('u-nd') == false &&
				other.hasClass('u-input') == false &&
				other.hasClass('u-time-input') == false
			) {
			...
			if (
				(input.hasClass('u-input') && other.hasClass('u-input')) ||
				(input.hasClass('g-combo') && other.hasClass('g-combo')) ||
				(input.hasClass('u-time-input') && other.hasClass('u-time-input'))
			) {
				otherInputCounts += 1;
			}
	...
	module.shrinkageLayout = function (options) {
		...
		//新增部分
		var showContent = function (show) {
			var dom = opts.dom;
			if (show == false) {
				element.find(dom).css('display', 'none');
			} else {
				element.find(dom).css('display', 'block');
			}
		};
	...
	module.getParams = function (name) {
		var search;
		if (window.location.search) {
			search = decodeURI(window.location.search).slice(1);
		} else {
			search = decodeURI(window.location.hash);
			if (search.indexOf('?') > -1) {
	...
	//设置主题色
	module.setTheme = function () {
		var father = window.opener || window.top;
		var theme = father.document.documentElement.style.getPropertyValue('--theme-primary');
		if (window.self !== father || !theme) {
			theme = theme
				? theme
				: unescape(
						sessionStorage.getItem('theme') ||
							...
							window.themeColor ||
							andy.getParams('theme') ||
							'#409eff'
				  );

			document.documentElement.style.setProperty('--theme-primary', theme);
			window.addEventListener('message', function (e) {
				if (e.data.type && e.data.type == 'setTheme') {
					var color = e.data.color
						? e.data.color
						: father.document.documentElement.style.getPropertyValue('--theme-primary');
					document.documentElement.style.setProperty('--theme-primary', color);
					var iframes = document.getElementsByTagName('iframe');
					for (var i = 0; i < iframes.length; i++) {
						iframes[i].contentWindow.postMessage({ type: 'setTheme' });
					}
				}
			});
		}
	...
	//水印
	module.watermark = function (option) {
		if (!option) {
			option = {};
		}
		...
		for (var i = 0; i < num; i++) {
			...
			item.style.cssText =
				...
				';width: ' +
				(100 / cols + '%') +
				(minWidth ? ';min-width: ' + parseInt(minWidth) + 'px' : '') +
				';height: ' +
				height +
				'px;line-height: ' +
				lineHeight +
...
$(document).ready(function () {
	...
	var option;
	if (window.opener && window.opener.webPageWatermark) {
		option = window.opener.webPageWatermark;
	} else if (window.top == window.self && window.webPageWatermark) {
		option = window.webPageWatermark;
	}
	if (option) {
		if (typeof option === 'string') {
			option = { html: option };
...
(function (window) {
	...
	module.getBrowser = function () {
		...
		var platform = (BROWSER.platform = {
			name:
				window.orientation != undefined ? 'iPod' : (pf.match(/mac|win|linux/i) || ['unknown'])[0],
			version: 0,
			iPod: 0,
			iPad: 0,
			iPhone: 0,
		...
		(s = ua.match(/windows ([\d.]+)/))
			? updateProperty('platform', 'win', toFixedVersion(s[1]))
			: (s = ua.match(/windows nt ([\d.]+)/))
			? updateProperty('platform', 'win', toFixedVersion(s[1]))
			: (s = ua.match(/linux ([\d.]+)/))
			? updateProperty('platform', 'linux', toFixedVersion(s[1]))
			: (s = ua.match(/mac ([\d.]+)/))
			? updateProperty('platform', 'mac', toFixedVersion(s[1]))
			: (s = ua.match(/ipod ([\d.]+)/))
			? updateProperty('platform', 'iPod', toFixedVersion(s[1]))
			: (s = ua.match(/ipad[\D]*os ([\d_]+)/))
			? updateProperty('platform', 'iPad', toFixedVersion(s[1]))
			: (s = ua.match(/iphone ([\d.]+)/))
			? updateProperty('platform', 'iPhone', toFixedVersion(s[1]))
			: (s = ua.match(/android ([\d.]+)/))
			? updateProperty('platform', 'android', toFixedVersion(s[1]))
			: 0;
		//============================================
		...
		(s = ua.match(/trident.*; rv\:([\d.]+)/))
			? updateProperty('browser', 'ie', toFixedVersion(s[1])) //IE11的UA改变了没有MSIE
			: (s = ua.match(/msie ([\d.]+)/))
			? updateProperty('browser', 'ie', toFixedVersion(s[1]))
			: (s = ua.match(/firefox\/([\d.]+)/))
			? updateProperty('browser', 'firefox', toFixedVersion(s[1]))
			: (s = ua.match(/chrome\/([\d.]+)/))
			? updateProperty('browser', 'chrome', toFixedVersion(s[1]))
			: (s = ua.match(/opera.([\d.]+)/))
			? updateProperty('browser', 'opera', toFixedVersion(s[1]))
			: (s = ua.match(/adobeair\/([\d.]+)/))
			? updateProperty('browser', 'adobeAir', toFixedVersion(s[1]))
			: (s = ua.match(/version\/([\d.]+).*safari/))
			? updateProperty('browser', 'safari', toFixedVersion(s[1]))
			: 0;

		//下面是各种微调
		//mobile safari 判断，可与safari字段并存
		(s = ua.match(/version\/([\d.]+).*mobile.*safari/))
			? updateProperty('browser', 'mobileSafari', toFixedVersion(s[1]))
			: 0;
		...
		(s = ua.match(/trident\/([\d.]+)/))
			? updateProperty('engine', 'trident', toFixedVersion(s[1]))
			: (s = ua.match(/gecko\/([\d.]+)/))
			? updateProperty('engine', 'gecko', toFixedVersion(s[1]))
			: (s = ua.match(/applewebkit\/([\d.]+)/))
			? updateProperty('engine', 'webkit', toFixedVersion(s[1]))
			: (s = ua.match(/presto\/([\d.]+)/))
			? updateProperty('engine', 'presto', toFixedVersion(s[1]))
			: 0;
...
(function (window) {
	...
	module.isSimilarColor = function (sHexColorA, sHexColorB, nOffset) {
		...
		var arrNumA = [
			parseInt(sHexColorA.substring(1, 3), 16),
			parseInt(sHexColorA.substring(3, 5), 16),
			parseInt(sHexColorA.substring(5, 7), 16)
		];
		// console.log(arrNumA,"aaa")
		var arrNumB = [
			parseInt(sHexColorB.substring(1, 3), 16),
			parseInt(sHexColorB.substring(3, 5), 16),
			parseInt(sHexColorB.substring(5, 7), 16)
		];
...
(function (window) {
	...
	module.copy = function (content, tip) {
		if ($('#copytextarea').length == 0) {
			$('body').append(
				"<textarea id='copytextarea' style='height:0; border:0; width:0; display:block; opacity:0'></textarea>"
			);
		}
...
(function (andy, $) {
	...
	andy.loading = function (target, text, callback) {
		var that = {};
		var content =
			'<div class="loading_bg_center"><span class="quarters-loader"></span><p id="loadingtext">' +
			text +
			'</p></div>';
...
(function (andy, $) {
	var tooltipRender;
	var htmlStr =
		'<div class="infobox" style="width:{{width}};top:{{top}}; left:{{left}}; bottom:{{bottom}}; right:{{right}}; display: none;">' +
...
(function (andy, $) {
	'use strict';
	var DBODYHOFFSET = 33 + 37 + 12 + 12 - 3, //head+bottom + bodypadiing
		DIALOGZ = 9000,
		...
		massageCfg = {
			...
			警告: {
				title: '警告!',
				icon: '',
				iconclass: 'f-color-warning',
				buttons: [{ text: '返回' }, { text: '继续' }]
			},
			提示: {
				title: '提示!',
				icon: '',
		...
		setPos = function (win, x, y) {
			...
			if (andy.IE() > 0 || andy.isFirefox()) {
				sTop =
					iframeW.ownerDocument.documentElement.scrollTop ||
					window.pageYOffset ||
					iframeW.ownerDocument.body.scrollTop;
			}
		...
		reSize = function (win, w, h) {
			...
			try {
				...
				if (data && data.options) {
					...
					bodyHeightOff =
						options.buttons && options.buttons.length > 0 ? DBODYHOFFSET : DBODYHOFFSET - 37;
		...
		headDragInit = function () {
			...
			moveRect = getWinTop().$(
				'<div dialogMoveRect="true" style="border: 1px dashed dodgerblue; position: absolute; "></div>'
			);
			//状态清除还原
			clear = function () {
				dragStart = false;
			...
			drgEnd = function () {
				var iframeW,
					options = getWinTop().$.data(wMain[0], 'an_dialog').options;
			...
			changeCursor = function (e) {
				...
				if (wMain) {
					if (wMain.attr('an_dialog_resize') === 'true') {
						...
						//右
						if (
							wMain.position().left + wMain.outerWidth() > e.x &&
							e.x > wMain.position().left + wMain.outerWidth() - 5
						) {
							dragType = dragType ? 2 : 3;
						}
						//下
						if (
							wMain.position().top + wMain.outerHeight() > e.y &&
							e.y > wMain.position().top + wMain.outerHeight() - 5
						) {
			...
			dmouseMove = function (e) {
				...
				if (!dragStart) {
					...
					l = l <= 0 ? 0.1 : l;
					l =
						l > moveRect.parent().width() - moveRect.width()
							? moveRect.parent().width() - moveRect.width()
							: l;
					t = t <= 0 ? 0.1 : t;
					...
					if (l <= 0.1 || t <= 0.1 || l >= moveRect.parent().width() - moveRect.width()) {
						if (
							wMain.offset().left > 0.1 &&
							wMain.offset().top > 0.1 &&
							wMain.offset().left < moveRect.parent().width() - moveRect.width()
						) {
							drgEnd();
							return;
						}
	...
	$.fn.an_dialog = function (options, params) {
		...
		return this.each(function () {
			...
			bindHandler = function (win) {
				...
				if (args.buttons) {
					for (i = 0; i < args.buttons.length; i++) {
						if (
							args.massage &&
							massageCfg[args.massage.type] &&
							massageCfg[args.massage.type].buttons
						) {
							win.callback = args.massage.callback;
							args.buttons[i].handler =
								args.buttons[i].handler ||
								function (e) {
									if (e.data.callback) {
			...
			organize = function () {
				...
				dom =
					'<div style="position: absolute; " an_dialog="true" an_dialog_resize = ' +
					args.resizeable +
					' class="animated m-panel window' +
					args.cls +
					'" style="display: none">' +
					//头部
				...
				getTools = function () {
					var s = '',
						cell,
						mode =
							'<button type="button" class="u-btn sm texture"><i class="iconfont tool-#type#"" href="javascript:void(0)" #attr#> #icon# </i></button>';
					if (args.collapsible) {
				...
				getBottons = function () {
					var s = '',
						i,
						mode = '<button  bt_index="#index#" class="#class#" > #text# #icon#</button>';
					if (args.massage) {
					...
					if (args.buttons) {
						...
						for (i = args.buttons.length - 1; i >= 0; i--) {
							s += mode
								.replace('#index#', i)
								.replace('#class#', 'u-btn sm ' + args.buttons[i].cls || 'texture')
								.replace('#text#', args.buttons[i].text || '');
				...
				getContent = function () {
					...
					if (args.url) {
						winbody =
							'<iframe frameborder="0" an_dialog="true" src = #content# class="panel-body"></iframe><div style="z-index: 1;position: absolute;left:50%;top:50%" class="loading_bg_center"><span class="quarters-loader"></span></div>';
						//先显示loading，载入页面
						c = args.url;
					} else {
						...
						if (args.data) {
							...
							if (args.massage) {
								c =
									'<div class="panel-massage-box"><i class="iconfont ' +
									massageCfg[args.massage.type].iconclass +
									' panel-massage-icon">' +
									massageCfg[args.massage.type].icon +
									'</i>' +
									'<div class="panel-massage">' +
									args.massage.content +
									'</div></div>';
							}
							if (args.html) {
								c = args.html;
	...
	$.fn.extend({
			...
			if (funstyle != '') {
				...
				if (tpwidth != '100%') {
					if (tl.lockTable.lockRow == 0) {
						box = $(
							"<div style = 'position:relative;clear:both;overflow-x:auto;height:100%;'></div>"
						);
					} else {
						box = $(
							"<div style = 'position:relative;clear:both;overflow-x:auto;overflow-y:hidden;height:100%;'></div>"
						);
					}
				}
				table.wrap(box);
		...
		an_tableLayout: function (element, pw, ph) {
			...
			var setStatus = function (element, status) {
				if (element.parent().is('td')) {
					...
					if (status) {
						...
						tr_data ? (tr_data.checked = true) : (tr_data = {});
						tr_data.checked = true;
						tr.data('tr_data', tr_data);
					} else {
						tr.removeClass('current');
						var tr_data = tr.data('tr_data');
						tr_data ? (tr_data.checked = false) : (tr_data = {});
			...
			var setNodeStatus = function (tr, fclass, clickt, isOpen) {
				...
				if (
					tclass &&
					tclass.substr(0, tclass.length - 1) == fclass.substr(0, fclass.length - 1) &&
					tclass.substr(tclass.length - 1, tclass.length) >
						fclass.substr(fclass.length - 1, fclass.length)
				) {
					// 操作文件夹
					if (isOpen) {
						$(tr).css('display', '');
					...
					if (isOpen) {
						...
						if (
							(preFirstGroup && preFirstGroup.children('td').hasClass('open') == false) ||
							current > clickIndex
						) {
							$(tr).css('display', 'none');
						}
		...
		an_tableLock: function (options) {
			...
			var box = $("<div id='" + boxid + "'></div>").scroll(function (e) {
				//在此处添加事件
			...
			if (tl.lockRow > 0) {
				...
				for (var r = 0; r < tl.lockRow; ++r) {
					//添加行锁定
					tr = table
						.find('thead tr:eq(' + r + ') >th')
						.addClass('LockRow')
						.addClass(tl.lockRowCss);
					if (!tr[r]) {
						// 头部不够 锁定body部分
						tr = table
							.find('tbody tr:eq(' + r + ') >td')
							.addClass('LockRow')
							.addClass(tl.lockRowCss);
					}
					...
					if (getNodeStatus(parentNode) > 0) {
						tb.find('thead tr').each(function (tr_index, tr) {
							$(tr)
								...
								.each(function (th_index, th) {
									if (
										tr_index == parseInt(parentNode.layer) &&
										th_index == parseInt(parentNode.th_index)
									) {
										var $th = $(th);
		...
		an_tableView: function () {
			...
			if (funstyle != '') {
				...
				// 对话框底部按钮
				var dialog_bottom =
					"<div class='u-p-l-bottom u-upload-bot' style = 'width:100%;height:40px;background:#EEE;position:absolute;bottom:0px;left:0px;_top:" +
					(_options.height - 45) +
					"px;_left:5px;_z-index:10000;'>" +
		...
		an_tabs: function () {
			...
			if (funstyle != '') {
				...
				var moveOptions = function (direction) {
					...
					var headWidth =
						head.width() - leftBarBtn.outerWidth() * 2 - head.find('li').length * 2 - 2;
					var ul = head.find('ul');
				...
				var start = function () {
					...
					head.find('li').each(function (i, e) {
						...
						$e.bind(_options.tabsEvent, function (event) {
							if (_options.onClickBefore(event)) {
								choose(i);
	...
	$.fn.extend({
		switchs: function () {
			...
			var stateInit = function ($element, flag) {
				...
				$element.addClass(stateClass).append(_icon).addClass('switchState');
			};
			Config = {
				/**
				 * 方法主程
				 ...
				 */
				setEnable: function () {
					$(this).children('input').prop('disabled', arguments[1]);
				},
				/**
				 * 开关状态
				 * @method setState
				...
				setState: function () {
					...
					$(this).children('input').prop('checked', arguments[1]);
				}
			};
...
(function (andy, $) {
	...
	var VERIFY = {
		...
		mobile: {
			...
			placeholder: '请输入手机号',
			check: function (e) {
				return /^[1][3, 4, 5, 6, 7, 8, 9][0-9]{9}$/.test(e.value);
			}
		},
		phone: {
			message: '座机号码不合法',
		...
		email: {
			...
			placeholder: '请输入邮箱',
			check: function (e) {
				return /^([A-Z0-9]+[_|\_|-|\-|\.]?)*[A-Z0-9]+@([A-Z0-9]+[_|\_|-|\-|\.]?)*[A-Z0-9]+\.[A-Z]{2,3}$/i.test(
					e.value
				);
			}
		},
		...
		norepeat: {
			...
			check: function (e) {
				var result = true,
					i,
					otherEl,
					norepeatName = e.getAttribute('norepeat');
	...
	var setCorrect = function (el) {
		//添加正确样式
		$(el).removeClass('err').addClass('cor');
		$(el).before(verifyIconDivCor);
	};
	var setError = function (el) {
		//添加错误样式
		$(el).removeClass('cor').addClass('err');
			...
			if (element.parentNode.className.indexOf('.m-tooltip') < 0) {
				...
				if ((rules + '').indexOf('required') > -1) {
					$(element)
						.parents('.u-formitem')
						.find('label')
						.append("<span class='f-color-danger'>*</span>");
				}
					...
					if (placeholder) {
						if (placeholder.indexOf('{{') > -1) {
							placeholder = placeholder.replace(
								...
								element.getAttribute(rules[rules.length - 1])
							);
						}
	...
	andy.addVerify = function (el, options) {
		...
		el.setAttribute(
			'verify',
			el.getAttribute('verify') ? el.getAttribute('verify') + ',' + checkName : checkName
		);
					...
					if (data['total'] && pagination.hasClass('promty') == false) {
						...
						obj = $.extend(true, data, tableOption, {
							onSelectPage: function (index, pageCounts) {
								...
								loadTable(
									_options.url + '?pageNumber=' + index + '&pageSize=' + pageCounts,
									'have'
								);
								// console.log("当前页:"+index+",显示条数:"+pageCounts);
								pageIndex = index;
			...
			if (funstyle != '') {
				...
				if (typeof _options.treeTitle == 'object') {
					for (var i = 0; i < _options.treeTitle.length; i++) {
						head.append(
							"<li index='1' selectbtn style='margin:5px 5px 0 0;'><a href='javascript:;'>" +
								_options.treeTitle[i] +
								'</a></li>'
						);
						leftframe.append(
							"<div class = 'g-max leftframe' style = 'position:relative;'>" +
								"<div class='panel-head f-b-b'>" +
								"<div class='panel-h-l'>" +
								"<strong class='title' treeTitle>" +
								_options.treeTitle[i] +
								'</strong>' +
								'</div></div>' +
			...
			if (funstyle != '') {
				...
				var addButton = function (node) {
					...
					if (_options.selects == false) {
						...
						inValueInput.attr({
							idValue: node.id,
							valueData: node.value || node.attrValue || node.attr
						});
				...
				if (_options.touchTargetType == 'input') {
					if (_options.combo.children().length == 0) {
						...
						_options.combo.append(
							"<div class='u-group " +
								_options.comboSize +
								"'><input class = 'item item-l u-input u-diseditor' readonly='readonly' verify = '" +
								_options.inputVerify +
								"' value = '" +
								_options.defaultValue +
								"' " +
								enableText +
								" combo = '" +
								_options.comboName +
								"'><i class='iconfont mark item item-r'></i></div>"
						);
						andy.formlayout();
					}
				} else if (_options.touchTargetType == 'button') {
					...
					if (_options.combo.children().length == 0) {
						_options.combo.append(
							"<div class='u-btn-eject' style='width:100%;' combo = '" +
								_options.comboName +
								"'><div class='u-btn-checkbox f-clear' id=" +
								selectId +
								'></div></div>'
						);
					}
			...
			if (funstyle != '') {
				...
				if (_options.showTargetType == 'list') {
					if (_options.combo.children().length == 1) {
						_options.combo.append(
							"<ul class = 'combo m-menu' combo = '" + _options.comboName + "'></ul>"
						);
					}
				} else if (_options.showTargetType == 'tree') {
					if (_options.combo.children().length == 1) {
						_options.combo.append(
							"<div class='combo but-tree' style='height:" +
								_options.showHeight +
								"px;' combo ='" +
								_options.comboName +
								"'></div>"
		...
		an_combo_bindEvent: function () {
			...
			if (funstyle != '') {
				...
				var createTree = function (id, set, nodes, op) {
					...
					// 插入树结构
					combo.showTarget.append(
						"<div class='g-h-max'><ul id='" + treeId + "' class='ztree'></ul></div>"
					);
					var tree = '';
					// 判断是否加载ztree结构json
					if (nodes) {
					...
					if (_options.showTargetType == 'list') {
						...
						for (var i = 0; i < data.length; i++) {
							...
							var li =
								...
								attrValueStr +
								" listId = '" +
								liData[_options.idvalue] +
								"' ><a>" +
								liData[_options.namevalue] +
								'</a></li>';
							lis += li;
						}
	...
	$.fn.extend({
		an_spinner: function () {
			...
			if (spi.is('input')) {
				...
				var valueInput =
					"<input type='text' class='item u-input nohover' name = '" +
					_options.inputName +
					"' style='text-align:center; width:0px; text-indent:0' value='" +
					_options.value +
					"' val>";
				var div =
					"<div class='u-group " +
					_options.spinnerSize +
					"' id = '" +
					spinner_id +
					"' style = '" +
					spinner_style +
...
(function (andy) {
	...
	var mouseMove = function (event) {
		...
		if (drags.length > 0) {
			if (dragStart && currentDrag) {
				...
				if (time > currentDrag.restrictedMoveTime) {
					//减少调用次数，防止卡死IE6-8
					lockTime = time;
	...
	andy.dragElement = function (options) {
		...
		var posCheck = function (argu, el) {
			el =
				el ||
				andy.findOutElement(argu.event.target, args.dragName, 'true', undefined, args.notDragEl);
			if (el) {
...
(function (andy) {
	var htmlStr =
		'<div  class="multi-select-box" style="position:relative;width: 100%;height:100%">' +
					...
					for (i; i < boxChildren.length - 1; i++) {
						...
						if (point.x > copyEl.offsetLeft && point.x < copyEl.offsetLeft + copyEl.offsetWidth) {
							if (point.y > copyEl.offsetTop && point.y < copyEl.offsetTop + copyEl.offsetHeight) {
								mouseIndex = index + 1;
								if (
									(!lastIndex && mouseIndex !== elIndex) ||
									(lastIndex && lastIndex !== mouseIndex)
								) {
	...
	andy.multiSelect = function (data) {
		...
		//关闭按钮事件
		var bindDeleteEvent = function (obj) {
			var index,
				box = $(obj).parent().parent();
			//执行删除回调
			if (data.onDelete) {
				//被删除的节点索引，数据节点
				index = parseInt($(obj).parent().attr('index'), 10);
				data.onDelete(index);
			}
			$(obj).parent().remove();
			changeIndex(box);
		...
		that.setEnable = function (isEnable) {
			if (!isEnable) {
				parent.append(
					"<div multiSelectMask style='position: absolute;width: 100%; height: auto;z-index: 2;background: #fff; opacity: 0.5; top: 0px;'></div>"
				);
		...
		that.addElement = function (element) {
			...
			var htmlStr =
				'<span multiSelect="true" index="' +
				data.list.length +
				'" data-id="' +
				(element.val || element.id || element.text) +
				'" class="option-block" nametext="' +
				(element.text || element.name) +
				'" ><span class="indexnum">' +
				data.list.length +
				'.</span> ' +
				(element.text || element.name) +
				'<i notdrag="true" class="iconfont"></i></span>';
			data.removeIndex
				? (htmlStr =
						'<span multiSelect="true" index="' +
						data.list.length +
			...
			parent.find('.multi-select-box span i:last').on('click', function (e) {
				bindDeleteEvent(this);
			});
			changeIndex(parent);
		};
...
(function (window) {
	...
	module.an_alertitem = function (
		...
		boxindex.prepend(
			"<div class='alert f-clear " +
				al_style +
				" f-m-b-xs animated fadeInDown alertnew' id = " +
				_options.id +
				' >' +
				item_icon +
				al_content +
	...
	module.an_alert = function (options) {
		...
		if (_options.audio != undefined && _options.audio != '') {
			window.top
				.$('body')
				.append("<audio id='audio' src='" + _options.audio + "' autoplay></audio>");
			setTimeout(function () {
				window.top.$('body').find('#audio').remove();
			}, 1000);
		}
...
(function ($) {
	...
	$.fn.an_radiobox.methods = {
		getValue: function (me, para) {
			...
			var val = '';
			if (checked.length) val = checked[0].value;
			return val;
		},
			...
			if (funstyle != '') {
					...
					for (var k in data) {
						...
						try {
							...
							if (li.children && li.children.length > 0 && $li.length > 0) {
								...
								$li
									.find('a')
									.append("<i class='iconfont arrow'>" + _options.childrenRightIcon + '</i>');
							}
							bindEvent($li);
						} catch (e) {
							alert(e);
						...
						try {
							...
							if (li.children && li.children.length > 0 && $li.length > 0) {
								...
								if (_options.downMenu) {
									$li
										.find('a')
										.append("<i class='iconfont arrow'>" + _options.childrenDownIcon + '</i>');
								}
							}
							bindEvent($li);
				...
				var setResize = function () {
					if (_options.isFitParent) {
						...
						var mpWidth =
							mp.parent().width() -
							otherWidth -
							parseInt(mp.css('margin-left')) -
							parseInt(mp.css('margin-right'));
						console.log(mpWidth);
				...
				var setOptionsBtn = function () {
					...
					right.unbind('click').on('click', function () {
						...
						movedis < -_options.menu.width() + mp.width()
							? (movedis = -_options.menu.width() + mp.width())
							: null;
	...
	$.fn.extend({
		an_sidemenu: function () {
			...
			if (funstyle != '') {
				...
				var hideThis = function (thisli) {
					// thisli.children("a").removeClass("active");
					thisli.find('ul').slideUp(300, function () {
						$(this).removeClass('show');
					});
				};
				...
				var showUl = function (thisli) {
					if (thisli.find('ul').length == 0 && thisli.data('children')) {
						...
						//thisli.find("ul").addClass("show");
						//setTimeout(function(){
						thisli.find('ul').slideDown(300).addClass('show');
						//},2000);
					} else {
						thisli.find('ul').eq(0).slideDown(300).addClass('show');
					}
				};
				...
				bindEvent = function (li) {
					...
					li.bind(_options.showEvent, function (e) {
						...
						thisli.parents('li').children('a').addClass('actived');
						//if(thisli.children("a").hasClass("parent")){
						//alert('true');
						thisli.siblings().children('a').removeClass('actived');
						//}
						// 阻止冒泡 准许事件发生
						e.stopPropagation();
						//  // 绑定侧边菜单操作
				...
				createUlHtml = function (parent) {
					...
					for (var k in data) {
						var li = data[k];
						try {
							var icon, url, target, str, active, clickDom, clickUrl;
							icon = '';
							url = 'javascript:;';
							active = '';
							if (li.icons) {
							...
							$('#' + clickDom)
								...
								.children('a')
								.addClass('actived');
							$('#an_center').find('#stage').attr('src', clickUrl);
							// 子节点数据绑定
							var $li = $('#' + li.id);
							$li.data('json', li);
							if (li.children && li.children.length > 0 && $li.length > 0) {
			...
			if (funstyle != '') {
				...
				scroll.showScrollBar = function () {
					if (!_options.alwaysShow) {
						...
						if (
							right.is(':hidden') == false &&
							bottom.is(':hidden') == false &&
							right.hasClass('edit') == false &&
							_options.isBarPlaceholder
						) {
							right.addClass('edit');
					...
					if (_options.overflowY == 'auto') {
						var sheight =
							parseInt(right.height() - bottomButton.height() * 2) *
							(top.height() / left.outerHeight());
					...
					if (_options.overflowX == 'auto') {
						var swidth =
							parseInt(bottom.width() - leftButton.height() * 2) *
							(top.width() / left.outerWidth());
			...
			if (funstyle != '') {
				...
				if (_options.touchTargetType == 'input') {
					if (_options.combo.children().length == 0) {
						...
						_options.combo.append(
							"<div class='u-group u-up-menu'><input class = 'item item-l u-input u-diseditor' readonly='readonly' verify = '" +
								_options.inputVerify +
								"' value = '" +
								_options.defaultValue +
								"' " +
					...
					if (_options.combo.children().length == 0) {
						...
						_options.combo.append(
							"<span class='u-but-group u-group-center'><p style='color:#000'>" +
								_options.defaultValue +
								"</p><i class='iconfont'></i><button type='button' class='u-but-button' combo = '" +
								_options.comboName +
								"'></button></span>"
						);
						_options.combo.toggleClass('u-btn-eject u-group-center');
		...
		an_inputcombo_creatShow: function () {
			...
			if (funstyle != '') {
				...
				if (_options.showTargetType == 'list') {
					if (_options.combo.children().length == 1) {
						_options.combo.append(
							"<ul class = 'combo u-down-menu' combo = '" + _options.comboName + "'></ul>"
						);
					}
				} else if (_options.showTargetType == 'tree') {
					if (_options.combo.children().length == 1) {
			...
			if (funstyle != '') {
				...
				function creatList(data) {
					...
					for (var i = 0; i < data.length; i++) {
						...
						icons =
							d.icons && d.icon != '' ? "<i class = 'iconfont homeicon'>" + d.icons + '</i>' : '';
						console.log();
